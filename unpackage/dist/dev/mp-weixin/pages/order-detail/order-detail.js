...
//
//
var xwCountDown = function xwCountDown() {__webpack_require__.e(/*! require.ensure | components/xw-CountDown/index */ "components/xw-CountDown/index").then((function () {return resolve(__webpack_require__(/*! @/components/xw-CountDown */ 497));}).bind(null, __webpack_require__)).catch(__webpack_require__.oe);};var _default = { data: function data() {return { orderDetail: {}, orderNo: '', mseconds: '' };}, components: { xwCountDown: xwCountDown }, computed: { enterTime: function enterTime() {var formatTime = (0, _moment.default)(parseInt(this.orderDetail.startTime * 1000)).format('yyyy-MM-DD HH:mm:ss');return formatTime;} }, methods: { getOrderDetail: function getOrderDetail(orderNo) {var _this = this;this.$api.getOrderDetail({ orderNo: orderNo }).then(function (res) {console.log(res);_this.orderDetail = res.orderDetail;if (_this.orderDetail.orderStatus === 1) {_this.setOrderInvalidationTime(_this.orderDetail.payTime);}});}, setOrderInvalidationTime: function setOrderInvalidationTime(payTime) {var date = payTime;date = date.substring(0, 19);date = date.replace(/-/g, '/'); //必须把日期'-'转为'/'
      var payTimestamp = new Date(date).getTime() / 1000;this.mseconds = payTimestamp + 900;console.log(this.mseconds);} }, onLoad: function onLoad(option) {console.log(option.orderNo);this.orderNo = option.orderNo;this.getOrderDetail(option.orderNo);} };exports.default = _default;
